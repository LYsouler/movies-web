...
app = Flask(__name__)
#app.config["SQLALCHEMY_DATABASE_URI"] = "mysql://user:user@127.0.0.1:3306/movie"
app.config["SQLALCHEMY_DATABASE_URI"] = "mysql+pymysql://user:user@127.0.0.1:3306/movie"
...
class User(db.Model):
    ...
    uuid = db.Column(db.String(255), unique=True)  # 序列码，唯一标志符
...
class Admin(db.Model):
    __tablename__ = "admin"
    id = db.Column(db.Integer, primary_key=True)  # 编号
    name = db.Column(db.String(100), unique=True)  # 管理员编号
...
if __name__ == "__main__":
    ...
    admin = Admin(
        name = "mooc",
        pwd = generate_password_hash("moocpwdmovie"),
        is_super = 0,
        role_id = 1
    )
